...
class ToolBar:
    ...
    def __init__(self, screen, speed=4, r_step=2, acceleration=2):
        ...
        self.background = image.load('menu_background.png')
...
class Game:
    ...
    def __init__(self, screen, speed, r_step, acceleration):
        ...
        self.board = [400, 150]
        self.font = font.Font('Retro_Gaming.ttf', 90)
        ...
        self.bg = image.load('map_resized.png')
        self.bg = transform.scale(self.bg, (self.width, self.height))  # 1:1.42
        self.screen.blit(self.bg, (0, 0))

        road = image.load('road.png')
        road = transform.scale(road, (1227, 864))
        self.m_road = mask.from_surface(road)
        self.m_road.invert()
        self.offset = 120

        self.tree = image.load('tree.png')
        self.tree = transform.scale(self.tree, (1227, 864))
        self.screen.blit(self.tree, (0, 0))

        obstacle = image.load('obstacle.png')
        obstacle = transform.scale(obstacle, (1227, 864))
        self.m_obstacle = mask.from_surface(obstacle)

        self.finish = image.load('finish.png')
        self.finish = transform.scale(self.finish, (1227, 864))
        self.screen.blit(self.finish, (0, 0))

        p_1 = image.load('car_1.png')
        self.r_1 = 0
        self.a_1 = 15
    ...
    def obstacle(self, n, dx, dy):
        flag = False
        match n:
            case 1:
                if self.m_1.overlap_area(self.m_obstacle, (-(self.positions[0][0] + dx), -(self.positions[0][1] + dy))) > 10:
                    flag = True
            case 2:
                if self.m_2.overlap_area(self.m_obstacle, (-(self.positions[1][0] + dx), -(self.positions[1][1] + dy))) > 10:
                    flag = True
        if flag:
    ...
    def draw_countdown(self, passed):
        center_x, center_y = self.bg.get_rect().center
        half_a, half_b = self.board[0] // 2, self.board[1] // 2
        draw.rect(self.bg, '#7F7F7F', (center_x - half_a, center_y - half_b, self.board[0], self.board[1]), 0, 10)
        draw.rect(self.bg, '#F0F0F0', (center_x - half_a, center_y - half_b, self.board[0], self.board[1]), 3, 10)
        seconds = self.font.render(passed, True, '#F0F0F0')
        t_rect = seconds.get_rect(center=self.bg.get_rect().center)
